...
import restApi.TestsApiConfig;
import restApi.Uris;
import restApi.CalculadoraWrapper;
...
public class AdminResourceFunctionalTesting {
    ...
    public void testBodyWrapper() {
        URI uri = UriComponentsBuilder.fromHttpUrl(URL_API).path(Uris.ADMINS).path(Uris.CALC2).build().encode().toUri();
        CalculadoraWrapper wrapper = new CalculadoraWrapper(9,3);

        RequestEntity<CalculadoraWrapper> requestEntity = new RequestEntity<>(wrapper, HttpMethod.POST, uri);

        String json = new RestTemplate().exchange(requestEntity, String.class).getBody();
        System.out.println(json);
        CalculadoraWrapper response = new RestTemplate().exchange(requestEntity, CalculadoraWrapper.class).getBody();
    ...
    public void testBodyWrapperList() {
        ...
        String json = new RestTemplate().exchange(uri, HttpMethod.GET, new HttpEntity<Object>(new HttpHeaders()), String.class).getBody();
        System.out.println(json);
        List<CalculadoraWrapper> response = Arrays.asList(new RestTemplate().exchange(uri, HttpMethod.GET, new HttpEntity<Object>(new HttpHeaders()),
        		CalculadoraWrapper[].class).getBody());
        System.out.println(response);
    }
    ...
    public void testErrorNotToken() {
        try {
            new RestBuilder<CalculadoraWrapper>(URL_API).path(Uris.ADMINS).path(Uris.ERROR).path("/66").get().build();
            fail();
    ...
    public void testErrorMalFormedToken() {
        try {
            new RestBuilder<CalculadoraWrapper>(URL_API).path(Uris.ADMINS).path(Uris.ERROR).path("/66").header("token", "kk").get().build();
            fail();
        } catch (HttpClientErrorException httpError) {
            assertEquals(HttpStatus.BAD_REQUEST, httpError.getStatusCode());
            System.out.println("ERROR >>>>> " + httpError.getMessage());
    ...
    public void testErrorNotExistToken() {
        try {
            new RestBuilder<CalculadoraWrapper>(URL_API).path(Uris.ADMINS).path(Uris.ERROR).path("/66").header("token", "Basic kk").get().build();
            fail();
        } catch (HttpClientErrorException httpError) {
    ...
    public void testErrorNotExistId() {
        try {
            new RestBuilder<CalculadoraWrapper>(URL_API).path(Uris.ADMINS).path(Uris.ERROR).path("/0").header("token", "Basic good").get().build();
            fail();
        } catch (HttpClientErrorException httpError) {
    ...
    public void testErrorOk() {
    	CalculadoraWrapper response = new RestBuilder<CalculadoraWrapper>(URL_API).path(Uris.ADMINS).path(Uris.ERROR).path("/666").header("token", "Basic good")
                .clazz(CalculadoraWrapper.class).get().build();
